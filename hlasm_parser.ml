...
let data_constant_subfield =
  ...
  if List.mem type_code valid_dc_types then
    ...
    (* modifiers: Lnn (length), Smm (scale), Emm (exponent) *)
...
let statement =
  ...
  | Some '*' -> 
      ...
      { label = None; stmt_type = Comment comment; remarks = None; pos }
  | _ ->
      option None (symbol_kind <* ws1 >>| Option.some) >>= fun label ->
      choice [
        assembler_instruction >>| (fun inst -> AssemblerInst inst);
        machine_instruction >>| (fun inst -> MachineInst inst);
      ] >>= fun stmt_type ->
      option None (ws1 *> take_while (fun c -> c <> '\n') >>| Option.some) >>| fun remarks ->
      { label; stmt_type; remarks; pos }

(* =============================================================================
   PROGRAM PARSER WITH CORRECT BLANK LINE HANDLING
   ============================================================================= *)

let program =
  let full_line_comment =
    ws *> char '*' *> take_while (fun c -> c <> '\n')
  in
  let blank_line =
    ws *> return ()
  in
  let eol =
    choice [ void (char '\n'); end_of_input ]
  in
  (* A line can be a statement, a comment, or blank. We only care about statements. *)
  let line_parser =
    choice [
      (ws *> statement >>| Option.some);
      (full_line_comment *> return None);
      (blank_line *> return None);
    ]
  in
  sep_by eol line_parser >>| fun results ->
...
let parse_hlasm input = 
  match parse_string ~consume:All program input with
  | Ok result -> Ok result
  | Error msg -> 
      let enhanced_msg = "HLASM parse error: " ^ msg in
      Error enhanced_msg
...
let test_cases = [
  "DC CL12'HELLO'";           (* Length modifier with character constant *)
  "DC 3CL5'ABC'";             (* Duplication factor *)
  "DC PL(4)S2'123'";          (* Program type and scale *)
  "DS 10F";                   (* Storage definition *)
  "G'<SO>漢字<SI>'";          (* DBCS graphic constant *)
  "C'混合<SO>DBCS<SI>文字'";   (* Mixed character constant *)
  "=CL8'LITERAL'";            (* Literal *)
  "LOOP    L     R1,DATA";    (* Labeled machine instruction *)
  "         USING *,R12";     (* USING statement *)
  "* This is a comment";      (* Comment line *)
  "MAIN    CSECT";            (* Control section *)
...
(* Test all parsers *)
let run_tests () =
  Printf.printf "Testing HLASM Parser...\n";
  Printf.printf "======================\n\n";
  
  List.iter (fun test_input ->
    ignore (test_parser program test_input);
    Printf.printf "\n"
  ) test_cases;
